  ...
  implements OnInit, AfterViewInit, OnChanges
{
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['listTable']) {
      const newData = changes['listTable'].currentValue;
      if (Array.isArray(newData)) {
        this.tableDataSource = new MatTableDataSource<any>(newData);
        this.tableDataSource.sort = this.sort;
        this.tableDataSource.paginator = this.paginator;
      } else {
        console.error('listTable input is not an array', newData);
      }
    }
  }
  ...
  implements OnInit, AfterViewInit, OnChanges
{
  ...
  ngOnChanges(changes: SimpleChanges): void {
    this.tableDataSource = new MatTableDataSource<any>(
      changes['listTable']?.currentValue
    );
    this.tableDataSource.sort = this.sort;
  }
